...
export default function App() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await askGemini(userMessage, knowledge);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const mappedData = data.map(row => ({
        question: row["Qual o seu erro?"] || row["Pergunta"] || row["question"],
        answer: row["Resposta"] || row["answer"]
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                      ...
                      <label className="w-full flex flex-col items-center justify-center px-4 py-6 bg-slate-50 text-slate-500 rounded-xl border-2 border-dashed border-slate-200 cursor-pointer hover:bg-slate-100 hover:border-blue-400 transition-all">
                        <FileUp size={24} className="mb-2" />
                        <span className="text-xs font-bold uppercase tracking-wider">Selecionar Arquivo</span>
